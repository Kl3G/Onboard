package com.example.Sep20Join;/*
  ...
  * 리스트 타입을 사용해야 .add() .remove() .size() 등의 메소드를 사용할 수 있다, 크기가 자동으로 조정된다. *
  ...
  26. Exception ex 와 IOException e 의 차이
    1) catch (Exception ex)
        예외 종류: Exception은 모든 예외를 포괄하는 최상위 클래스입니다. 따라서 Exception으로 설정하면 모든 종류의 예외가 잡힐 수 있습니다.
        출력 방식: System.out.println(ex.toString())은 예외 내용을 간단하게 한 줄로 출력합니다.
                예외의 요약된 정보만 나오므로 문제 원인을 확인하기엔 간단하지만 제한적입니다.
        용도: 모든 예외를 간단히 로그로 남기고, 예외 상황을 크게 분석하지 않아도 될 때 사용합니다.
    2) catch (IOException e)
        예외 종류: IOException은 파일 입출력 중 발생할 수 있는 예외를 의미합니다.
                파일 작업이 실패했을 때 이 예외가 발생하며, 이 외의 예외는 여기서 잡히지 않습니다.
        출력 방식: e.printStackTrace()는 예외 발생 지점과 호출된 메서드 순서까지 자세히 출력합니다.
                따라서 문제의 정확한 위치와 원인을 추적하기에 좋습니다.
        용도: 파일이나 입출력 오류를 디버깅할 때 구체적인 예외 내용을 알고 싶을 때 사용합니다.


  27. @Transactional 에 대해
    설명: 데이터베이스 작업을 할 때, 여러 개의 작업이 한꺼번에 실행될 수 있습니다.
        @Transactional은 이 여러 작업을 하나로 묶어서 **"모두 성공하거나, 모두 실패해야 한다"**는 규칙을 만듭니다.
    이유: 중간에 문제가 생기면 데이터가 이상해질 수 있기 때문에,
        모든 작업이 문제 없이 끝나야 실제로 데이터에 적용되도록 하는 장치입니다.
    예: 여러 항목을 장바구니에 넣고 결제할 때, 결제가 실패하면 모든 항목이 취소되고,
        결제가 성공해야만 모든 항목이 주문 완료되는 원리와 비슷합니다.


  28. @Modifying 에 대해
    설명: 일반적으로 데이터베이스를 읽는 쿼리(SELECT)와 달리, 데이터를 변경하거나 삭제하는 쿼리(UPDATE, DELETE)를 사용할 때 필요합니다.
